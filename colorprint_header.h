...
typedef enum { TAG_OPEN, TAG_CLOSE } _cp_tag_t;

typedef enum { FOREGROUND, BACKGROUND } _cp__cp_color_type_t;
...
static const char* _cp_bg_color_strings[] = {
    [RESET] = "49",         [RED] = "41",          [GREEN] = "42",
    [YELLOW] = "43",        [BLUE] = "44",         [MAGENTA] = "45",
    [CYAN] = "46",          [WHITE] = "47",        [LIGHTRED] = "101",
...
static void _cp_colorstack_push(_cp_colorstack_t* stack,
                                _cp_ansi__cp_color_t color) {
  if (stack->ptr >= 16) {
    return;
  }
    ...
    if (command.valid) {
      ...
      if (enable && command.valid) {
        if (set_color || set_bgcolor) {
          char format[32];
          sprintf(format, "\x1b[%s;%sm", _cp_fg_color_strings[color],
                  _cp_bg_color_strings[bgcolor]);
...
static const char* _cp_spinner_value_0[] = {"|", "/", "-", "\\", NULL};

static const char* _cp_spinner_value_1[] = {
    "\x1b(0t\x1b(A", "\x1b(0w\x1b(A", "\x1b(0u\x1b(A", "\x1b(0v\x1b(A", NULL};

static const char* _cp_spinner_value_2[] = {
    "\x1b(0j\x1b(A", "\x1b(0m\x1b(A", "\x1b(0l\x1b(A", "\x1b(0k\x1b(A", NULL};

static const char* _cp_spinner_value_3[] = {"\x1b(0m\x1b(A", "\x1b(0t\x1b(A",
                                            "\x1b(0n\x1b(A", "\x1b(0u\x1b(A",
                                            "\x1b(0j\x1b(A", NULL};

static const char* _cp_spinner_value_4[] = {
    "\x1b(0m\x1b(A", "\x1b(0x\x1b(A", "\x1b(0l\x1b(A",
    "\x1b(0q\x1b(A", "\x1b(0k\x1b(A", "\x1b(0x\x1b(A",
    "\x1b(0j\x1b(A", "\x1b(0q\x1b(A", NULL};

static const char** _cp_spinner_values[] = {
    _cp_spinner_value_0, _cp_spinner_value_1, _cp_spinner_value_2,
    _cp_spinner_value_3, _cp_spinner_value_4};

static char** _cp_spinner_value = (char**)_cp_spinner_value_0;

void spinner_start(int color, unsigned int type, const char* fmt, ...) {
  _cp_spinner_value =
      (char**)_cp_spinner_values[type % (sizeof(_cp_spinner_values) /
                                         sizeof(_cp_spinner_values[0]))];
  _cp_spinner_state = 0;
...
void progress_update(int color) {
  ...
  printf_color(1, "\x1b(0x\x1b(B %3d%%",
               100 * _cp_progress_state / _cp_progress_max);
